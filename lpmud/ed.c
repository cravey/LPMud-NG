...
extern	void	prntln(char *str, int vflg, int lin);
extern	void	putcntl(char c, FILE *stream);
...
int egets(char *str, int size, FILE *stream)
{
	...
	str[count-1] = EOS;
	if(c != NL && !feof(stream) && !ferror(stream)) {
...
int subst(regexp *pat, char *sub, int gflg, int pflag)
{
	int	nchngd = 0;
	char	*txtptr;
