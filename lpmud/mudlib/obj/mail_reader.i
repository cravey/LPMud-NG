...
static string arr_messages;	
static int loaded;		
static int curr_mess;		
...
load_player() {
    loaded = 1;
    if (!restore_object("room/post_dir/" + this_player()->query_real_name()) ||
      messages == "" || messages == 0) {
	arr_messages = 0;
	messages = "";
	return;
    ...
    if (new_mail) {
	new_mail = 0;
	save_object("room/post_dir/" + this_player()->query_real_name());
    }
...
send_mail(dest, subj, mess, cc) {
    ...
    messages = "";
    restore_object("room/post_dir/" + dest);
    if (messages == 0)
	messages = "";
    messages += this_player()->query_name() + "\n**\n" + subj + cc +
	"Date: " + extract(ctime(time()), 4, 9) + "\n\n" + mess + "\n**\n";
    new_mail = 1;
    save_object("room/post_dir/" + dest);
}
