...
TWO_EXIT("players/ringer/pyramid/ehall2","west",
         "players/ringer/pyramid/fmfalse","east",
         "East/west hallway",
"The hallway ends to the east in a large room.  The walls are smooth and\n"+
"uniform and somewhat dusty.\n",0)

ex_init() {
    add_action("search"); add_verb("search");
    if (call_other(this_player(),"query_ghost")) return;
    if (call_other("players/ringer/pyramid/ehall1","query_stone")==0) {
        write(
