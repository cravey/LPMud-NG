...
short() { return "End of northeast tunnel"; }

long() {
    write(
"This is the end of a short tunnel.  The way leads back to the southwest.\n"+
...
init() {
    ...
    add_action("seast"); add_verb("southwest");
...
seast() {
    call_other(this_player(),"move_player",
               "southwest#players/ringer/pyramid/ncor2");
    return 1;
}
...
north() {
    if (!passage) return 0;
    call_other(this_player(),"move_player",
               "north#players/ringer/pyramid/negem");
    return 1;
}
...
search(str) {
    if (!str) str = "room";
    if (str == "room") {
        if (call_other("players/ringer/pyramid/nwtun","query_stones")==13){
