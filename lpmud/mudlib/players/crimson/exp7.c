...
id(str) { 
    return str == "potion of hallucinations" || str == "potion of hallucination" || str == "potion";
}
...
object flask;
   ...
   write("You quaff down the multi-colored potion.\n");
...
read(str) {
  if (str == "label" && i == 0 || str == "label on multi-colored potion" && i == 0) {
  write("The label reads: XXX.\n");
  return 1;
  } else
  if (!str) {
  return 0;
