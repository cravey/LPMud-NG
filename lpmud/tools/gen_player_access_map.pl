...
my $root = "mudlib";
my $out  = shift @ARGV || "mudlib/player_access_map.dot";
...
for my $file (@files) {
    ...
    # Heuristic for owner-locked wizard rooms.
    if ($src =~ m{^players/([^/]+)/}) {
        my $wiz = $1;
        if ($txt =~ /query_real_name\s*\(\s*\)\s*!=\s*"(\Q$wiz\E)"/s ||
            $txt =~ /query_real_name\s*\(\s*this_player\s*\(\s*\)\s*\)\s*!=\s*"(\Q$wiz\E)"/s)
        {
            $restricted_node{$src} = 1;
        }
    }
...
for my $k (keys %edges) {
    ...
    next if $restricted_node{$src} || $restricted_node{$dst};
    push @{$adj{$src}}, [$dst, $dir];
}
...
print {$out_fh} "digraph player_access_map {\n";
...
for my $k (sort keys %edges) {
    my ($src, $dst, $dir) = split /\0/, $k, 3;
    next unless $reachable{$src} && $reachable{$dst};
    next if $restricted_node{$src} || $restricted_node{$dst};
...
my $node_count = scalar(keys %reachable);
my $edge_count = 0;
for my $k (keys %edges) {
    my ($src, $dst, $dir) = split /\0/, $k, 3;
    next unless $reachable{$src} && $reachable{$dst};
...
print "Wrote $out ($node_count reachable nodes, $edge_count edges)\n";
