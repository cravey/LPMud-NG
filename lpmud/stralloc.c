...
struct shared_string_hdr {
	struct shared_string_hdr *next;
	short refs;
	short pad;
};

#define HDR(str)	((struct shared_string_hdr *)((char *)(str) - sizeof(struct shared_string_hdr)))
#define	NEXT(str)	((char *)HDR(str)->next)
#define	SET_NEXT(str, v)	(HDR(str)->next = (struct shared_string_hdr *)(v))
#define	REFS(str)	(HDR(str)->refs)
	...
	while (curr) {
	    search_len++;
			...
			if (prev) { /* not at head of list */
			    SET_NEXT(prev, NEXT(curr));
			    SET_NEXT(curr, base_table[h]);
			    base_table[h] = curr;
			    }
...
static char *alloc_new_string(char *string) {
	struct shared_string_hdr *hdr;
	char *s;
	size_t len = strlen(string);
	int h = StrHash(string);

	hdr = (struct shared_string_hdr *)xalloc(sizeof(struct shared_string_hdr) + len + 1);
	s = (char *)(hdr + 1);
	strcpy(s, string);
	REFS(s) = 0;
	SET_NEXT(s, base_table[h]);
	base_table[h] = s;
	num_distinct_strings++;
	bytes_distinct_strings += 4 + (strlen(s) +3) & ~3;
	overhead_bytes += sizeof(struct shared_string_hdr);
	return(s);
}
...
void free_string(char *str) {
	...
	{ int align;
	align = (int)(((uintptr_t)str) - sizeof(int) - sizeof(short)) & WORD_B_MASK;
	if (align)
